export default async function handler(req, res) {
  ...
  const SUPA_URL = process.env.SUPA_URL || 'https://hyhufujgxcvcttfxhwbd.supabase.co';
  const SUPA_KEY = process.env.SUPA_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imh5aHVmdWpneGN2Y3R0Znhod2JkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzg1NzcwNzgsImV4cCI6MjA5NDE1MzA3OH0.ssIvQZAvwi5SLYUMmiMHUBkMvRcSLmMVMU_OylwswVA';
  ...
  try {
    ...
    const rows = [
      ...alerts.critical.map(v => buildRow(v, '#dc2626', '⚠ 10 DAYS — CRITICAL')),
      ...alerts.warning.map(v => buildRow(v, '#d97706', '30 days — Warning')),
      ...alerts.upcoming.map(v => buildRow(v, '#2563eb', '50 days — Upcoming')),
    ].join('');
...
<body style="margin:0;padding:0;background:#f5f0e8;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif">
  <div style="max-width:640px;margin:32px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08)">
    <div style="background:#1e1c1b;padding:24px 32px">
      ...
      <div style="font-size:11px;color:#8a8278;letter-spacing:1px;text-transform:uppercase;margin-top:4px">MOT Alert — ${new Date().toLocaleDateString('en-GB',{day:'numeric',month:'long',year:'numeric'})}</div>
    </div>
    <div style="padding:24px 32px">
      <p style="font-size:15px;color:#1e1c1b;margin-bottom:20px">
        The following vehicles have MOT expiry dates coming up. Please arrange inspections as soon as possible.
      </p>
    ...
    <div style="padding:16px 32px;background:#f5f0e8;font-size:11px;color:#8a8278">
      This is an automated notification from Workship. <a href="https://workship-nine.vercel.app" style="color:#e8a020">Open Workship</a>
    </div>
  </div>
</body>
</html>`;
    ...
    const subject = alerts.critical.length > 0
      ? `⚠ CRITICAL MOT Alert — ${alerts.critical.length} vehicle(s) due in 10 days`
      : alerts.warning.length > 0
      ? `MOT Warning — ${alerts.warning.length} vehicle(s) due in 30 days`
      : `MOT Reminder — ${alerts.upcoming.length} vehicle(s) due in 50 days`;
    ...
    const emailRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'Workship <onboarding@resend.dev>',
        to: TO_EMAIL,
        subject,
        html
      })
